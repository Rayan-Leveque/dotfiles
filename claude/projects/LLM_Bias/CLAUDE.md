...
1. **Logprobs sans CoT** — **IMPLÉMENTÉ (2026-07-16)** : `src/evaluation/run_comparative_logprobs.py` (standalone, cf. README § 3b-bis) + `call_llm_logprobs()` dans `utils/llm_client.py`. Prompt « Réponds uniquement par A ou B », lit P(A)/P(B) dans les top-logprobs du premier token, écrit `data/results/comparative_logprobs_<model>.csv` (p_a, p_b, p_french, p_french_norm, flag = P(A)+P(B) ≥ 0,95). Thinking désactivé d'office. Smoke test 2026-07-16 sur Qwen3-4B (serveur perso port 2487, GPUs occupés → Gemma pas encore lancée) : 20/20 lignes valides, P(A)+P(B) ≈ 1 partout ; Qwen3-4B sature sur la position (P(A) ≈ 0,9999 quel que soit l'ordre) mais la granularité sous saturation subsiste (P(B) ~ 10⁻⁵) — la moyenne sur les deux ordres annule bien la position. Reste à lancer sur les modèles de l'étude (Gemma, Qwen3.6-27B en local ; DeepSeek/Novita dès que les clés API sont dans le `.env`).
